...
def process_motif_row(motif_row):
    ...
    print(motif_row.target)
    motif_path = os.path.join('./motif_scaffolding/targets/', motif_row.target.lower()+'.pdb')
...
def load_contigs_by_test_case(inpaint_df):
    ...
    for _, row in inpaint_df.iterrows():
        name = str(row.target)
        if name in ['il7ra_gc', 'rsv_site4', '6EXZ_short', '6EXZ_med', '6EXZ_long']:
            continue
