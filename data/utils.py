...
def parse_feats(feats, scale_factor=1.):
    ca_idx = residue_constants.atom_order['CA']
    feats['bb_mask'] = feats['atom_mask'][:, ca_idx]
    bb_pos = feats['atom_positions'][:, ca_idx]
    bb_center = np.sum(bb_pos, axis=0) / (np.sum(feats['bb_mask']) + 1e-5)
    centered_pos = feats['atom_positions'] - bb_center[None, None, :]
    scaled_pos = centered_pos / scale_factor
    feats['atom_positions'] = scaled_pos * feats['atom_mask'][..., None]
    feats['bb_positions'] = feats['atom_positions'][:, ca_idx]
    return feats
...
def process_chain(chain: Chain, chain_id: str) -> Protein:
    ...
    for res in chain:
        ...
        for atom in res:
            if atom.name not in residue_constants.atom_types: continue
...
def get_gt_value_from_pdb(file_path, centering=False):
    parser = PDB.PDBParser(QUIET=True)
    if file_path[-4:] == '.pdb':
        structure = parser.get_structure(os.path.basename(file_path).split('_')[0], file_path)
    else:
        structure = parser.get_structure(' ', StringIO(file_path))
    ...
    if centering:
        complex_feats = du.parse_feats(complex_feats)
    else:
        complex_feats['bb_mask'] = complex_feats['atom_mask'][:, 1]
        complex_feats['bb_positions'] = complex_feats['atom_positions'][:, 1]
        
    return complex_feats


def get_gt14_value_from_pdb(file_path, num_atom, centering=False):
    parser = PDB.PDBParser(QUIET=True)
    if file_path[-4:] == '.pdb':
        structure = parser.get_structure(os.path.basename(file_path).split('_')[0], file_path)
    else:
    ...
    atom_pos = complex_feats['atom_positions']
    atom_mask = complex_feats['atom_mask']
    _dims = atom_pos.shape
    
    if _dims[-2] != 14: # 'N', 'CA', 'C', 'O', 'CB'
        
        atom_pos_ = np.zeros((_dims[0], 14, 3))
        atom_pos_[:, :3] = atom_pos[:, :3, :14]
        atom_pos_[:, 3] = atom_pos[:, 4, :14]
        atom_pos_[:, 4] = atom_pos[:, 3, :14]
        
        atom_mask_ = np.zeros((_dims[0], 14))
        atom_mask_[:, :3] = atom_mask[:, :3]
        atom_mask_[:, 3] = atom_mask[:, 4]
        atom_mask_[:, 4] = atom_mask[:, 3]
    
    complex_feats['atom_positions'] = atom_pos_[:, :num_atom]
    complex_feats['atom_mask'] = atom_mask_[:, :num_atom]
    complex_feats['b_factors'] = complex_feats['b_factors'][:, :num_atom]
