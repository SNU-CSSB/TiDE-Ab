...
class EdgeTransition(nn.Module):
    ...
    def forward(self, node_embed, edge_embed):
        ...
        edge_bias = torch.cat([torch.tile(node_embed[:, :, None, :], (1, 1, num_res, 1)),
                               torch.tile(node_embed[:, None, :, :], (1, num_res, 1, 1)),], axis=-1)
...
class InvariantPointAttention(nn.Module):
    ...
    def __init__(
        ...
        # if self._ipa_conf.use_2d_hotspot:
        #     print('use_2d_hotspot')
        #     self.c_z = ipa_conf.c_z + 1
    ...
    def forward(
        ...
        # [*, H, N_res, N_res]
        a = torch.matmul(permute_final_dims(q, (1, 0, 2)), permute_final_dims(k, (1, 2, 0))) # [*, H, N_res, C_hidden], [*, H, C_hidden, N_res]
...
class IpaScore(nn.Module):
    ...
    def __init__(self, model_conf, diffuser):
        ...
        for b in range(ipa_conf.num_blocks):
            ...
            self.trunk[f'skip_embed_{b}'] = Linear(self._model_conf.node_embed_size, self._ipa_conf.c_skip, init="final")
    ...
    def forward(self, init_node_embed, edge_embed, input_feats):
        ...
        rot_score = self.diffuser.calc_rot_score(init_rigids.get_rots(), curr_rigids.get_rots(), input_feats['t'])
        rot_score = rot_score * node_mask[..., None]

        curr_rigids = self.unscale_rigids(curr_rigids)
        trans_score = self.diffuser.calc_trans_score(init_rigids.get_trans(), curr_rigids.get_trans(), 
                                                     input_feats['t'][:, None, None], use_torch=True,)
